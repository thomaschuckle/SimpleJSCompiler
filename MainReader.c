...
integer isNumber(const string ns);
void startReader(string, string, character, integer, integer);
...
integer mainReader(integer argc, string* argv) {
	...
	string input = argv[2];
	character mode = MODE_FIXED;
	integer size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
void startReader(string program, string input, character mode, integer size, integer increment) {

	BufferPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	integer loadSize = 0;		/* the size of the file loaded in the buffer */
	character symbol;			/* symbol read from input file */
